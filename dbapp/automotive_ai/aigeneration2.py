#!/usr/bin/env python3
"""
Refactored Grok-based data enhancer (robust JSON parsing).
...
# ---------------- PROMPT (UNCHANGED) ----------------
PROMPT_TEMPLATE = """
You are an IoT and Embedded Systems Cybersecurity Threat Analysis AI.
For each numbered vulnerability description, generate one JSON object
with exactly the following 17 fields:

["vendor","product_name","title","attack_path","interface","protocol",
 "tools_used","types_of_attack","level_of_attack","affected_component",
 "damage_scenario","cia","impact","feasibility","countermeasures",
 "firmware_version","library_name"]
...
- Do NOT hallucinate technical details. Only mention specific devices, components,
  protocols, or hardware interfaces when the description clearly supports it.
- If IoT/embedded relevance is unclear, keep all device-specific fields
  (product_name, affected_component, firmware_version, library_name) as "Unknown".
...
- damage_scenario = concise real-world device/system impact.
- countermeasures = practical, cybersecurity-focused mitigations.
- interface = e.g., Wi-Fi, Ethernet, BLE, Zigbee, USB, UART, JTAG, Serial, etc.
- protocol = e.g., HTTP, HTTPS, MQTT, Modbus, CAN, Zigbee, BLE, etc.
- affected_component = e.g., firmware, bootloader, RTOS, web UI, radio stack, OTA module, etc.
...
EXPECTED_FIELDS = [
    "vendor", "product_name", "title", "attack_path", "interface", "protocol", "tools_used", "types_of_attack",
    "level_of_attack", "affected_component", "damage_scenario", "cia", "impact", "feasibility",
    "countermeasures", "firmware_version", "library_name"
]
...
def fetch_unprocessed_rows(conn, limit: int) -> List[Tuple]:
    with conn.cursor() as cur:
        cur.execute(
            ...
            "FROM classified_iot_embedded_vulnerabilities WHERE processed = false ORDER BY id ASC LIMIT %s;",
            (limit,)
        )
        rows = cur.fetchall()
    return rows
...
def insert_batch_and_mark_processed(conn, rows_with_fields: List[Tuple]):
    ...
    with conn.cursor() as cur:
        insert_query = """
        INSERT INTO iot_embedded_vulnerabilities (
        ...
        cur.execute("UPDATE classified_iot_embedded_vulnerabilities SET processed = true WHERE id = ANY(%s);", (ids,))
    conn.commit()
        ...
        while True:
            ...
            for row, res in zip(rows, results):
                ...
                rows_to_insert.append((
                    ...
                    nf["vendor"], nf["product_name"], nf["title"], nf["attack_path"], nf["interface"],
                    nf["protocol"], nf["tools_used"], nf["types_of_attack"], nf["level_of_attack"],
                    nf["affected_component"], nf["damage_scenario"], nf["cia"], nf["impact"], nf["feasibility"],
                    nf["countermeasures"], nf["firmware_version"], nf["library_name"]
                ))
