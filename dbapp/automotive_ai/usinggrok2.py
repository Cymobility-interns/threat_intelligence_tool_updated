...
def insert_into_classified(records):
    if not records:
        return 0
    cols = list(SCHEMA_FIELDS)
    values = [[rec.get(col) for col in cols] for rec in records]
    sql = f"""
        INSERT INTO classified_iot_embedded_vulnerabilities ({','.join(cols)})
        VALUES %s
...
5. Use "IoT Vulnerability" for embedded devices that are connected to the internet,
   ...
   smart meters, connected cameras, wearables, or any device designed to exchange
   data with a cloud service or remote application).

6. Use "Embedded Systems Vulnerability" for standalone embedded devices that are
   NOT primarily internet-connected and NOT industrial control systems (e.g.,
   firmware-based controllers inside appliances, offline medical devices, or
   microcontroller-based products that operate independently).

7. Use "Operational Technology (OT) Vulnerability" ONLY for embedded devices that
   are part of industrial control, automation, or critical infrastructure systems,
   such as PLCs, SCADA components, industrial controllers, factory automation
   equipment, or control systems used in utilities, manufacturing, or process plants.
...
"""
        ...
        try:
            ...
            for item in parsed:
                ...
                if label in ["IoT Vulnerability", "Embedded Systems Vulnerability", "Operational Technology (OT) Vulnerability"]:
                    iot_embedded_records.append(rec)
                    print(f"[SELECTED] ID {rec['id']}, {cve_id} => {label}")
                else:
                    print(f"[SKIPPED] ID {rec['id']}, {cve_id} => {label}")
...
def run_full_pipeline(batch_size=50):
    ...
    while True:
        ...
        iot_embedded_records = classify_batch(source_rows, sub_batch_size=10)

        inserted = insert_into_classified(iot_embedded_records)
